...
def start_server():
    try:
        ...
        bad_port = True
        chatroom = socket.socket(socket.AF_INET,
                                 socket.SOCK_STREAM)  # create a socket with the address family AF_INET and socket type SOCK_STREAM
        ...
        while True:  # Always accept new connections
            ...
            if len(clients) < 10:
                ...
                while (user_name_exists):
                    try:
                        chosen_user_name = conn.recv(4096).decode(
                            'utf-8')  #the first message from the client is the name
                    except socket.error as e:
                    ...
                    if chosen_user_name not in user_names_set:
                        ...
                        conn.send(("True," + str(len(clients))).encode(
                            'utf-8'))  #Communicate to the client that the name is not taken
                        client = {}
                        ...
                        for client in clients:
                            if client['user_name'] != chosen_user_name:
                                client['connection'].send(
                                    (chosen_user_name + " has joined the chatroom.\n").encode('utf-8'))
    ...
    while (user_has_left == False and server_closing == False):
        try:
            ...
            if msg == '/connection_closed':  # A client will send /connection_closed to acknowledge the server shutdown.s
                ...
                if len(
                        clients) == 0:  #If all clients have acknowledged the shutdown and themselves closed, the chatroom can close
                    chatroom.close()
                    print("Goodbye.")
                    sys.exit()
            ...
            if msg == '/exit' or msg == "/quit" or msg == "/part":
                ...
                user_names_set.remove(connection['user_name'])
                mutex.release()
                connection['connection'].send("/bye".encode(
                    'utf-8'))  # Tell the client that it is no longer listening to it. The client will disconnect itself
                user_has_left = True
