...
def set_up_connection():
    ...
    while(bad_port):
        try:
            serverName = raw_input('Server (i.e. rc02xcs213.managed.mst.edu): ')
...
def get_user_name(conn):
    bad_user_name = True
    while(bad_user_name):
        userName = raw_input('Choose a user name (i.e. Fred): ')
        conn.send(userName)
        valid_name = conn.recv(4096)
